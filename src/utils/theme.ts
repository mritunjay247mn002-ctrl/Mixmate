import drinkImageGradients from '../data/drink-image-gradients.json';
...
type DrinkImageGradientEntry = {
  gradient?: string[];
  accent?: string;
};

/**
 * Hero/card gradient: per-slug palette from bundled `drink-image-gradients.json`
 * (generated by `npm run images:apply`), otherwise category default from `gradFor`.
 */
export function heroGradFor(slug: string, category?: string): [string, string, string] {
  const row = (drinkImageGradients as Record<string, DrinkImageGradientEntry>)[slug];
  const g = row?.gradient;
  if (g && g.length >= 2) {
    const c0 = g[0];
    const c1 = g[1];
    const c2 = g[2] ?? row.accent ?? c1;
    return [c0, c1, c2];
  }
  return gradFor(category);
}
