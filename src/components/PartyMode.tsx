...
const VIBES: {
  ...
  },
  {
    ...
    filter: (d) => (d.difficulty ?? 'easy') === 'easy' && (d.prep_time ?? 5) <= 4,
  },
  {
    key: 'zero',
          ...
          return (
            <Pressable key={d.id} onPress={() => onPick(d)}>
              ...
              >
                ...
                <Text style={styles.vEmoji}>{d.emoji}</Text>
                <Text numberOfLines={2} style={styles.vName}>
                  {d.name}
                </Text>
                <View style={styles.vMeta}>
                  <Ionicons name="time-outline" size={10} color="#fff" />
                  <Text style={styles.vMetaTxt}>{d.prep_time ?? 5}m</Text>
                </View>
              </View>
            </Pressable>
          );
